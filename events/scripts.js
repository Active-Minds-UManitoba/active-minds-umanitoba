const sortedEventList = Object.values(eventList).sort(
...
function insertEvents(events, containerId) {
  ...
  if (container && container.classList.contains("active")) {
    ...
    if (events.length === 0) {
      container.innerHTML =
        '<p class="no-events" style="color: var(--tamarind); font-size: clamp(1rem,2vw,2rem); text-align: center; margin: 20px 0; padding: 2rem; border-radius: 5px;  justify-self: center; align-self: center; grid-column: 1 / -1;">No events to show. Maybe the aliens took them!</p>';
    }
  ...
  container.addEventListener("click", function (event) {
    ...
    if (cardType) {
      ...
      const targetItem = document.querySelector(
        '.event-list__item[data-container="' + itemMap[type] + '"]',
      );
      if (targetItem) {
        targetItem.click();
      }
...
function updateBackgroundColor() {
  ...
  const r = Math.round(
    startColor.r + (endColor.r - startColor.r) * scrollFraction,
  );
  const g = Math.round(
    startColor.g + (endColor.g - startColor.g) * scrollFraction,
  );
  const b = Math.round(
    startColor.b + (endColor.b - startColor.b) * scrollFraction,
  );
